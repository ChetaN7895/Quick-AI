...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      toast.loading("Generating article, please wait...");

      const prompt = `Write an article about ${input} in ${selectedLength.text}`;
      const token = await getToken();

      const { data } = await axios.post(
        "/api/ai/generate-articles",
      ...
      // ✅ Flexible handling for different backend response formats
      const article =
        data.content ||
        data.data?.content ||
        data.output ||
      ...
      if (article) {
        ...
        toast.dismiss();
        toast.error(data.message || "No content received from server.");
      }
    } catch (error) {
